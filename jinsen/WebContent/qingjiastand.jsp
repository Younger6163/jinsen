...
<body>
<main>
    <div class="home-tab"><i class="tab-i"></i> 所在位置：<span>请假扣款标准界面</span><a href="qingjia.jsp" style="float:right;margin-right:20px;color:black;"><span>返回</span></a></div>
    <div class="find-top">
        <p class="p-tail"><i class="i-tail"></i> 该页面可添加，删除，修改请假类型的扣费标准</p>
    </div>
    <div class="find-top1">
        <table class="top-table">
            <tr><td></td></tr>
            <tr><td class="top-table-label">请假类型：</td><td><input type="text" id="types"></td><td class="top-table-label">扣费标准：</td><td><input type="text" id="stand"></td></tr>
...
<script>
		...
		success:function(data){
			...
			totalCount = data.length;
			 for(var i=0;i<data.length;i++){
						var o = data[i];
						var str = 
						"<tr><th>"+o.leavetype_id
						+"</th><th>"+o.leavetype_type
						+"</th><th>"+o.leavetype_stand
						+"</th><th><button type='button' class='btn btn-info' id='user_edit' onclick='editorselect("+o.leavetype_id+")' style='margin-right:10px;'data-toggle='modal' data-target='#myModal'>编辑</button>"
						+"<button type='button' class='btn btn-danger' id='user_del' onclick='typedelete("+o.leavetype_id+")'>删除</button>"
						+"</th></tr>"
						htmlstr+=str; 
			}
			$("#body1").html(htmlstr); 
    	...
    	$.ajax({
    		...
    		success:function(data){
    			...
    			totalCount = data.length;
    			 for(var i=0;i<data.length;i++){
    						var o = data[i];
    						var str ="<form action='#'><div class='form-group'><label for='addname'>请假类型</label><input type='text' id='type' class='form-control' value="+o.leavetype_type+"></div><div class='form-group'><label for='addpassword'>扣费标准</label><input type='text' id='stands' class='form-control' value="+o.leavetype_stand+"></div></form><div class='modal-footer'><button type='button' class='btn btn-default' data-dismiss='modal'>关闭</button><button type='button' class='btn btn-primary' onclick='updatetype("+o.leavetype_id+")'>提交</button></div>"
    						htmlstr+=str; 
    			}
    			$("#body2").html(htmlstr); 
    		},
    ...
    function table1() {
        ...
        $('#query').bootstrapTable({
            ...
            singleSelect:false,
            cacahe:false,
            url:"leaveovertimeServlet?action=typeselect",
            dataType:"json",
            ...
            }, 
            {
                    ...
                    field: 'leavetype_type',
                    title:  '请假类型名称'
            }, {
                    field: 'leavetype_stand',
                    title:  '扣费标准'
            },{
                     title: '操作',
                     field: 'leavetype_id',
